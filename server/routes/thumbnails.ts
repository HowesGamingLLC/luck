...
async function downloadThumbnail(url: string, cacheKey: string): Promise<string | null> {
  try {
    ...
    if (uint8Array.length > MAX_FILE_SIZE) {
      console.error(`File too large after download: ${uint8Array.length} bytes`);
      return null;
    }
...
export const preloadThumbnails: RequestHandler = async (req, res) => {
  try {
    ...
    const results = await Promise.allSettled(
      urls.slice(0, 20).map(async (url: string) => { // Limit to 20 URLs
        try {
          ...
          return {
            ...
            thumbnailUrl: filename ? `/thumbnails/${filename}` : '/placeholder.svg',
          };
        } catch (error) {
          return {
            url,
    ...
    res.json({
      ...
      successful: processed.filter(r => r.status === 'downloaded' || r.status === 'cached').length,
    });
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      if (cacheEntry) {
        ...
        res.setHeader('Last-Modified', new Date(cacheEntry.cachedAt).toUTCString());
      }
