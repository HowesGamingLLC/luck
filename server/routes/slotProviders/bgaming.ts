...
export class BGamingProvider extends BaseSlotProvider {
  ...
  async getGameById(gameId: string): Promise<SlotGame | null> {
    try {
      const response = await fetch(
        `${this.baseUrl}/games/${gameId}?operator_id=${this.operatorId}`,
        {
          headers: {
            Authorization: `Bearer ${this.apiKey}`,
            "Content-Type": "application/json",
  ...
  async getPlayerBalance(playerId: string): Promise<ProviderBalance | null> {
    try {
      const response = await fetch(
        `${this.baseUrl}/players/${playerId}/balance?operator_id=${this.operatorId}`,
        {
          headers: {
            Authorization: `Bearer ${this.apiKey}`,
            "Content-Type": "application/json",
  ...
  async getTournaments() {
    try {
      const response = await fetch(
        `${this.baseUrl}/tournaments?operator_id=${this.operatorId}`,
        {
          headers: {
            Authorization: `Bearer ${this.apiKey}`,
            "Content-Type": "application/json",
  ...
  async getPromotions() {
    try {
      const response = await fetch(
        `${this.baseUrl}/promotions?operator_id=${this.operatorId}`,
        {
          headers: {
            Authorization: `Bearer ${this.apiKey}`,
            "Content-Type": "application/json",
