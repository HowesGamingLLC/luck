...
export class TournamentEngine extends EventEmitter {
  ...
  private scheduleTournament(config: {
    ...
    const tournament: Tournament = {
      ...
      schedule: {
        ...
        recurringTime: config.recurringPattern ? this.formatTime(config.tournamentStart) : undefined
      },
      participants: [],
      maxPlayers: config.maxPlayers,
      currentLevel: 0,
      blindSchedule: config.gameType === 'poker' ? this.generateBlindSchedule() : undefined,
  ...
  private getDefaultStructure(gameType: Tournament['gameType']): TournamentStructure {
    switch (gameType) {
      case 'poker':
        return {
          startingChips: 10000,
  ...
  private generatePayoutStructure(maxPlayers: number): PayoutLevel[] {
    ...
    const percentages = [
      40, 25, 15, 10, 6, 4, // Top 6
      3, 2, 2, 1.5, 1.5, 1.5, 1.5, 1, 1, 1 // 7th-16th
    ];

    for (let position = 1; position <= Math.min(payoutPositions, percentages.length); position++) {
      payouts.push({
        position,
  ...
  private updateTournamentLevels(): void {
    for (const tournamentId of this.activeTournaments) {
      ...
      const timeSinceStart = Date.now() - (tournament.startTime?.getTime() || 0);
  ...
  private advanceTournamentLevel(tournamentId: string): void {
    ...
    // Check for breaks
    if (tournament.structure.breakFrequency > 0 &&
        tournament.currentLevel % tournament.structure.breakFrequency === 0) {
      this.emit('tournamentBreak', { tournamentId, level: tournament.currentLevel });
    }
  ...
  public createTournament(config: {
    ...
    const tournament: Tournament = {
      ...
      structure: { ...this.getDefaultStructure(config.gameType), ...config.structure },
      schedule: {
  ...
  public registerPlayer(tournamentId: string, player: Player): { success: boolean; error?: string } {
    const tournament = this.tournaments.get(tournamentId);
    if (!tournament) {
  ...
  public startTournament(tournamentId: string): { success: boolean; error?: string } {
    const tournament = this.tournaments.get(tournamentId);
    if (!tournament) {
      return { success: false, error: 'Tournament not found' };
    }
  ...
  public eliminatePlayer(tournamentId: string, playerId: string): { success: boolean; error?: string } {
    const tournament = this.tournaments.get(tournamentId);
    if (!tournament) {
      return { success: false, error: 'Tournament not found' };
    }
    ...
    player.position = tournament.participants.filter(p => p.eliminated).length;

    this.emit('playerEliminated', { tournamentId, playerId, position: player.position });
  ...
  public finishTournament(tournamentId: string): TournamentResult | null {
    ...
    const finalStandings = tournament.participants.sort((a, b) => (a.position || 999) - (b.position || 999));
    ...
    const result: TournamentResult = {
      ...
      finalStandings,
      payouts,
      duration: tournament.endTime && tournament.startTime 
        ? Math.floor((tournament.endTime.getTime() - tournament.startTime.getTime()) / 60000)
        : 0,
  ...
  private distributePrizes(tournament: Tournament, finalStandings: TournamentPlayer[]): 
    { playerId: string; amount: number; currency: 'GC' | 'SC' }[] {
    const payouts: { playerId: string; amount: number; currency: 'GC' | 'SC' }[] = [];
    ...
    tournament.payoutStructure.forEach(payout => {
      if (payout.position <= finalStandings.length && payout.amount) {
        ...
        const gcRatio = tournament.buyIn.gc / (tournament.buyIn.gc + tournament.buyIn.sc || 1);
        const scRatio = tournament.buyIn.sc / (tournament.buyIn.gc + tournament.buyIn.sc || 1);
  ...
  private scheduleNextRecurrence(tournament: Tournament): void {
    if (!tournament.schedule.isRecurring || !tournament.schedule.recurringPattern) return;
  ...
  public getTournaments(
    ...
    return tournaments.sort((a, b) => a.schedule.tournamentStart.getTime() - b.schedule.tournamentStart.getTime());
  }
  ...
  public getPlayerTournaments(playerId: string): Tournament[] {
    return Array.from(this.tournaments.values())
      .filter(t => t.participants.some(p => p.id === playerId));
  }

  public updatePlayerChips(tournamentId: string, playerId: string, chips: number): void {
    const tournament = this.tournaments.get(tournamentId);
    if (!tournament) return;
