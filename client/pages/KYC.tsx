...
export default function KYC() {
  ...
  const getKYCStatusInfo = () => {
    if (!user) return { icon: XCircle, text: "Unknown", color: "text-gray-500", description: "" };
    ...
    switch (user.kycStatus) {
      case "approved":
        return {
          ...
          description: "Your identity has been successfully verified. You can now make withdrawals.",
        };
      case "pending":
        return {
          icon: Clock,
      ...
      case "rejected":
        return {
          ...
          text: "Rejected",
          color: "text-red-500",
          description: "Your verification was rejected. Please check the reasons below and resubmit.",
        };
      default:
        return {
          icon: AlertTriangle,
          ...
          description: "Complete the verification process to enable withdrawals and increase your limits.",
        };
    }
  };
  ...
  const completedSteps = documents.filter(doc => doc.status === "uploaded").length;
  const progress = user?.kycStatus === "not_submitted" ? (currentStep - 1) * 33.33 + (completedSteps / documents.length) * 33.33 : 100;
          ...
          <Card className="glass text-center">
            ...
            <CardContent className="space-y-6">
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
                <h3 className="font-semibold text-green-700 mb-2">You're all set!</h3>
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] py-8">
      <div className="container max-w-4xl">
        ...
        {/* Status Banner */}
        <Card className={`mb-8 border-${kycStatus.color.split('-')[1]}-500 bg-${kycStatus.color.split('-')[1]}-500/5`}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4 text-xs">
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${currentStep > 1 ? 'bg-green-500' : currentStep === 1 ? 'bg-blue-500' : 'bg-gray-300'}`} />
                  <span>Personal Info</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${currentStep > 2 ? 'bg-green-500' : currentStep === 2 ? 'bg-blue-500' : 'bg-gray-300'}`} />
                  <span>Document Upload</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${currentStep > 3 ? 'bg-green-500' : currentStep === 3 ? 'bg-blue-500' : 'bg-gray-300'}`} />
        ...
        <Tabs value={`step-${currentStep}`} onValueChange={(value) => setCurrentStep(parseInt(value.split('-')[1]))}>
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="step-2" disabled={!validateStep(1)}>Documents</TabsTrigger>
            <TabsTrigger value="step-3" disabled={!validateStep(1) || !validateStep(2)}>Review</TabsTrigger>
          </TabsList>
            ...
            <Card className="glass">
              ...
              <CardContent className="space-y-6">
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    Make sure all information matches exactly with your government-issued ID. 
                    Any discrepancies may delay the verification process.
                  </AlertDescription>
                </Alert>
          ...
          <TabsContent value="step-2">
            <Card className="glass">
              <CardHeader>
                ...
                <CardDescription>
                  Upload clear photos of your documents. Make sure all text is readable and images are well-lit.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {documents.map((doc) => (
                  <div key={doc.type} className="border rounded-lg p-4 space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        {doc.type === "id" && <FileText className="h-5 w-5 text-blue-500" />}
                        {doc.type === "address" && <FileText className="h-5 w-5 text-green-500" />}
                        {doc.type === "selfie" && <Camera className="h-5 w-5 text-purple-500" />}
                        <div>
                          <h4 className="font-medium">{doc.name}</h4>
                          <p className="text-sm text-muted-foreground">
                            {doc.type === "id" && "Driver's license, passport, or national ID"}
                            {doc.type === "address" && "Utility bill, bank statement, or lease agreement (within 3 months)"}
                            {doc.type === "selfie" && "Take a selfie holding your ID next to your face"}
                          </p>
                        </div>
                      </div>
          ...
          <TabsContent value="step-3">
            <Card className="glass">
              <CardHeader>
                ...
                <CardDescription>
                  Please review your information and documents before submitting for verification
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <Alert>
                  <Info className="h-4 w-4" />
                  <AlertDescription>
                    By submitting this verification, you confirm that all information provided is accurate 
                    and that you are the person depicted in the uploaded documents. False information may 
